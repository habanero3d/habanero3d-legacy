...
#pragma region IBoundingVolume
	...
	template <typename Real>
	std::unique_ptr<IBoundingVolume<Real>> AABB<Real>::clone() const
	{
		return unique(new AABB<Real>(*this));
	}
#pragma endregion
...
#pragma region OBB
	template <typename Real>
	bool AABB<Real>::contains(const OBB<Real> &obb) const
	{
		vector3<Real> points[8];
		obb.calculatePoints(points);

		// zawiera się wtw wszystkie wierzchołki się zawierają (chyba nie ma prostszego testu)
		for(int i = 0; i < 8; i++)
	...
	template <typename Real>
	bool AABB<Real>::collides(const OBB<Real> &obb) const
	{
		// przesunięte do OBB
		OBB<Real> me(*this);
		return me.collides(obb);
	}

	template <typename Real>
	InterferenceType AABB<Real>::interferes(const OBB<Real> &obb) const
	{
	...
	template <typename Real>
	bool AABB<Real>::growToContain(const OBB<Real> &obb)
	{
		vector3<Real> points[8];
		obb.calculatePoints(points);
		bool result = false;
	...
	template <typename Real>
	bool AABB<Real>::growToContain(const OBB<Real> &obb, const matrix4<Real> &transformation)
	{
		vector3<Real> points[8];
		obb.calculatePoints(points);
		for(int i = 0; i < 8; i++)
