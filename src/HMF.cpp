...
namespace Habanero
{
	namespace HMF2
	{
		Habanero::HeightMapMesh::InitInfo loadFromFile(const char *fileName, Resource::Mapping &materials, Resource::Mapping &textures)
		{
			...
			Specific_ptr<Texture> heightMap = TextureManager::getInstance().getResource(textures[hmInfo.textureId]);
			heightMap->load();

			HASSERT(width >= 2 && height >= 2);

			GenericGeometry<StaticVertex>::InitInfo initInfo;
			auto &vertices = initInfo.vertices;
			...
			{
				byte *line = (byte*)heightMap->data + z * (heightMap->height - 1) / (height - 1) * heightMap->width * 4;
				for (uint x = 0; x < width; x++)
				{
					StaticVertex v;
					v.position.x = x * sxy;
					v.position.y = line[x * (heightMap->width - 1) / (width - 1) * 4] * maxY;
					v.position.z = z * sxy;
					v.texCoord.x = x * repX;
					v.texCoord.y = z * repY;
