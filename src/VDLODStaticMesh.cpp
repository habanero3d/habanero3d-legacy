...
namespace Habanero
{
	VDLODStaticMesh::VDLODStaticMesh(const std::string &filename,
	...
	VDLODStaticMesh::~VDLODStaticMesh()
	{
		unload();
	}

	uint VDLODStaticMesh::getNumVertexSplits() const
	{
		return numVertexSplits;
	}

	// helper, copypasta from sphere::planeInterference
	bool VDLODStaticMesh::isOverOrThrough(const plane<float>& plane, const vector3f &center, float radius)
	{
	...
	GenericGeometry<StaticVertex> * VDLODStaticMesh::getGeometry(const Frustum<float>& ofru, const matrix4f &parentToLocal)
	{
	...
	bool VDLODStaticMesh::load()
	{
		if (getLoadState() != Discovered)
			return false;
		InitInfo initInfo = VDLODTMF::loadFromFile(filename.c_str(), materials, textures);
