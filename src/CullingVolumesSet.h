...
namespace Habanero
{
	ENUM(SceneMembership, VisibleObject, ShadowCaster, None);
	
	template <typename Real> class ShadowCasterVolume;
	...
	private:
		...
		std::vector<ShadowCasterVolume<float> > shadowCasterVolumes;
	public:
		...
		void addPointLightSource(vector3f worldLightPos);
		
		SceneMembership getSceneMembership(const Spatial& spatial);
	};
}
