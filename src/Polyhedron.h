...
namespace Habanero
{
	...
	public:
		...
		const std::vector<plane<Real>>& getPlanes() const { return planes;}
		const std::vector<vector3<Real> >& getVertices() const { return vertices;}
		const std::vector<PolyhedronEdge>& getEdges() const { return edges;}
